...
$('#mytab').bootstrapTable({
    ...
    url: "/advert/advertList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="advert"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(advertid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/advert/deleteAdvert/' + advertid,
            dataType: 'json',
...
function edit(name) {
    $.post("/advert/findAdvert/" + name,
        function (data) {
            $("#updateform").autofill(data);
            if(data.url){
                $('#img_s').attr('src', data.url); //图片链接（base64）
                $("#url_Img").val(data.url);
            }
        },
        "json"
    );
...
function updatestatus(id, status) {
    $.post("/advert/updateStatus/" + id + "/" + status,
        function (data) {
            if (status == 0) {
                if (data.message == "ok") {
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/advert/advertList'});
})
...
$("#updateSta").click(function () {
    layer.confirm('确认要执行批量修改角色状态吗？', function (index) {
        $.post(
            "/advert/deleteManyAdvert",
            {
                "manyId": $("#statusId").val(),
