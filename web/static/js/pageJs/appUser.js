...
$('#mytab').bootstrapTable({
    ...
    url: "/appUser/appUserList",//要请求数据的文件路径
    ...
    columns: [
        ...
        },
        {
            ...
            sortable: true,
            formatter: function (value, row, index) {
                return '<a   data-toggle="modal" title="点击查看详情" alt="点击查看详情" data-id="' + row.id + '"  data-target="#fs_modal" onclick="return fs('+row.id+')"><i class="glyphicon glyphicon-heart" style="color: red;">'+value+'</i></a>';
            }
        },
        {
            title: '签名',
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="appUser"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
                ...
                var p = '<a title="重置密码" href="javascript:void(0);" id="appUser"  onclick="return updatePwd(' + row.id + ')"><i class="glyphicon glyphicon-repeat" alt="重置密码" style="color:green">重置密码</i></a>';
...
function fs(val) {
    var html = '<div class="col-sm-6 col-md-3">\n' +
        '                        <a class="thumbnail">\n' +
        '                            <img src="~" style="width: 102px;height: 102px" class="viewer-item"  />\n' +
        '                        </a><h4 align="center" style="margin-top: -15px" >~</h4>\n' +
        '                    </div>';
    $.post("/appUser/getFenSi/" + val,
        function (data) {
            var htmls = "";
            for(var i=0;i<data.length;i++){
                htmls = htmls + html.replace("~",data[i].faceImg).replace("~",data[i].name);
            }
            $("#imgs").html(htmls);
...
function del(appUserid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/appUser/deleteAppUser/' + appUserid,
            dataType: 'json',
...
function updatePwd(name) {
    $.post("/appUser/resetPwd" ,
        {
            id:name
        },
        function (data) {
            if(data.result=="success"){
                layer.alert(data.message+"密码为:dt888888", {icon: 1});
            }else{
...
function updatestatus(id, status) {
    $.post("/appUser/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    ...
    $('#mytab').bootstrapTable('refresh', {
        ...
        query:{
            ...
            max:$("#max").val(),
            remark:$("#remark").val(),
            status:$("#appUserStatus").val()
        }
    });
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/appUser/appUserList'});
}
$("#update").click(function () {
    $.post(
        "/appUser/appUserUpdateSave",
