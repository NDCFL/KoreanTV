...
$('#mytab').bootstrapTable({
    ...
    columns: [
        ...
        },
        {
            ...
            sortable: true,
            formatter: function (value, row, index) {
                return '<span style="color: green">'+value+'个</i>';
            }
        },
        {
            title: '类型描述',
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="shop"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(shopid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/shop/deleteShop/' + shopid,
            dataType: 'json',
...
function updatestatus(id, status) {
    $.post("/shop/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/shop/shopList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/shop/shopList'});
}
