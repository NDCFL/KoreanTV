...
$('#mytab').bootstrapTable({
    ...
    url: "/userFriend/userFriendList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="userFriend"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(userFriendid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/userFriend/deleteUserFriend/' + userFriendid,
            dataType: 'json',
...
function edit(name) {
    $.post("/userFriend/findUserFriend/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/userFriend/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/userFriend/userFriendList'});
})
function getList(val) {
    $('#mytab').bootstrapTable('refresh', {
        url: '/userFriend/userFriendList'
        ,query:{
            id:val
        }
    });
...
$("#update").click(function () {
    $.post(
        "/userFriend/userFriendUpdateSave",
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/userFriend/deleteManyUserFriend",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
