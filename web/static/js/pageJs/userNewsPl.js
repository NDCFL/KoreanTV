...
$('#mytab').bootstrapTable({
    ...
    url: "/userNewsPl/userNewsPlList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="userNewsPl"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(userNewsPlid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/userNewsPl/deleteUserNewsPl/' + userNewsPlid,
            dataType: 'json',
...
function edit(name) {
    $.post("/userNewsPl/findUserNewsPl/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/userNewsPl/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    ...
    $('#mytab').bootstrapTable('refresh', {
        url: '/userNewsPl/findUserNewsPlList',
        query:{
            createTime:start,
            endTime:end,
...
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/userNewsPl/userNewsPlList'});
}
$("#update").click(function () {
    $.post(
        "/userNewsPl/userNewsPlUpdateSave",
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/userNewsPl/deleteManyUserNewsPl",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
