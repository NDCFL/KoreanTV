...
$('#mytab').bootstrapTable({
    ...
    url: "/vedio/vedioList",//要请求数据的文件路径
    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                return '<img src="'+value+'" class="viewer-item" style="width: 80px;height: 80px"/>';
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                if(value==0){
                    return '<span style="color: green">已完结</span>';
                }else{
        ...
        },
        {
            ...
            sortable: true,
            formatter: function (value, row, index) {
                value = (value.toString()==null?"暂无说明":value.toString());
                if(value.length<10){
                    return '<a   data-toggle="modal" title="点击查看详情" alt="点击查看详情" data-id="\'' + row.id + '\'" data-target="#remark_modal" onclick="return remarks(\'' + value.toString() + '\')">'+value.toString().substr(0,10)+'</a>';
                }else{
                    return '<a   data-toggle="modal" title="点击查看详情" alt="点击查看详情" data-id="\'' + row.id + '\'" data-target="#remark_modal" onclick="return remarks(\'' + value.toString() + '\')">'+value.toString().substr(0,10)+"..."+'</a>';
                }
            }
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="vedio"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(vedioid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/vedio/deleteVedio/' + vedioid,
            dataType: 'json',
...
function edit(name) {
    $.post("/vedio/findVedio/" + name,
        function (data) {
            $("#updateform").autofill(data);
            document.getElementById("demo_1").src=$("#face_Img").val();
        },
        "json"
    );
}
function updatestatus(id, status) {
    $.post("/vedio/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {
        url: '/vedio/findVedioList'
        ,query:{
            vedioModuleId:$("#vedioModuleId_").val(),
            vedioTypeId:$("#vedioTypeId").val(),
            title:$("#title").val(),
            rate:$("#rate").val(),
            isVip:$("#isVip").val(),
            isLast:$("#isLast").val()
...
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/vedio/vedioList'});
}
