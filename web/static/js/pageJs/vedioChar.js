...
$('#mytab').bootstrapTable({
    ...
    url: "/chars/getCharList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="chars"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function queryParams(params) {
    ...
    return {
        ...
        searchVal: title,
        vedioDiscussId:$("#discussId").val()
    }
}
function del(charsid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/chars/deleteChar/' + charsid,
            dataType: 'json',
            success: function (data) {
                if (data.message == '删除成功!') {
                    layer.alert(data.message, {icon: 6});
...
function updatestatus(id, status) {
    $.post("/chars/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/chars/charsList'});
}
$("#update").click(function () {
    $.post(
        "/chars/charsUpdateSave",
