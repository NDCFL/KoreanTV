...
$('#mytab').bootstrapTable({
    ...
    url: "/vedioDiscuss/vedioDiscussList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            sortable: true,
            formatter: function (value, row, index) {
                return '<a   data-toggle="modal" title="点击查看回复" alt="点击查看回复" data-id="\'' + row.id + '\'"  onclick="return huifu(\'' + row.id + '\')"><span style="color: #2fdb2e;font-size: 18px">'+value+'</span>条</a>';
            }
        }
        ,
        {
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="vedioDiscuss"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
                ...
                var p = '<a title="回复列表" onclick="return huifu(' + row.id + ')"><i class="glyphicon glyphicon-gift"  style="color:#ec4fff">查看回复</i></a> ';
                return e+d + f+p;
            }
        }
    ],
...
function  huifu(val) {
    location.href="/vedioDiscuss/chars/"+val;
}
//请求服务数据时所传参数
...
function del(vedioDiscussid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/vedioDiscuss/deleteVedioDiscuss/' + vedioDiscussid,
            dataType: 'json',
...
function edit(name) {
    $.post("/vedioDiscuss/findVedioDiscuss/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/vedioDiscuss/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {
        url: '/vedioDiscuss/findVedioDiscussList'
        ,query:{
            vedioId:$("#vedioId").val(),
...
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/vedioDiscuss/vedioDiscussList'});
}
$("#update").click(function () {
    $.post(
        "/vedioDiscuss/vedioDiscussUpdateSave",
...
$("#add").click(function () {
    $.post(
        "/vedioDiscuss/vedioDiscussAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/vedioDiscuss/deleteManyVedioDiscuss",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
