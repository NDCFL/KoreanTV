...
$('#mytab').bootstrapTable({
    ...
    url: "/vedioLookHistory/vedioLookHistoryList",//要请求数据的文件路径
    ...
    columns: [
...

        {
            ...
            formatter: function (value) {
                return '<span style="color: red;">'+value+'秒</span>';
            }
        }
        ,
        {
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="vedioLookHistory"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(vedioLookHistoryid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/vedioLookHistory/deleteVedioLookHistory/' + vedioLookHistoryid,
            dataType: 'json',
...
function edit(name) {
    $.post("/vedioLookHistory/findVedioLookHistory/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/vedioLookHistory/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {
        url: '/vedioLookHistory/findVedioLookHistoryByList',
        query:{
            vedioId:$("#vedioId").val(),
            vedioSectionId:$("#vedioSectionId_").val(),
            userName:$("#userName").val()
        }
    });
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/vedioLookHistory/vedioLookHistoryList'});
}
$("#update").click(function () {
    $.post(
        "/vedioLookHistory/vedioLookHistoryUpdateSave",
...
$("#add").click(function () {
    $.post(
        "/vedioLookHistory/vedioLookHistoryAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/vedioLookHistory/deleteManyVedioLookHistory",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
