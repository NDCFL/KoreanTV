...
$('#mytab').bootstrapTable({
    ...
    url: "/vedioModule/vedioModuleList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="vedioModule"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(vedioModuleid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/vedioModule/deleteVedioModule/' + vedioModuleid,
            dataType: 'json',
...
function edit(name) {
    $.post("/vedioModule/findVedioModule/" + name,
        function (data) {
            $("#updateform").autofill(data);
            if(data.imgUrl){
                $('#img_s').attr('src', data.imgUrl); //图片链接（base64）
                $("#url_Img").val(data.imgUrl);
            }
        },
        "json"
    );
...
function updatestatus(id, status) {
    $.post("/vedioModule/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/vedioModule/vedioModuleList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/vedioModule/vedioModuleList'});
}
$("#update").click(function () {
    ...
    var urlImg = $("#url_Img").val();
    if(!title){
        layer.alert("剧模块名称不能为空!", {icon: 2});
        return false;
    }
    if(!urlImg){
        layer.alert("图标不能为空", {icon: 2});
        return;
    }
    if(!level){
        layer.alert("剧模块级别不能为空!", {icon: 2});
        return false;
    }
    if(parseFloat(level)<0 || parseFloat(level)>10){
        layer.alert("剧模块级别范围位0-10!", {icon: 2});
        return false;
    }
    $.post(
        "/vedioModule/vedioModuleUpdateSave",
...
$("#add").click(function () {
    ...
    $.post(
        "/vedioModule/vedioModuleAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/vedioModule/deleteManyVedioModule",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
