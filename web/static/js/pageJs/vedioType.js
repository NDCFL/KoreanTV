...
$('#mytab').bootstrapTable({
    ...
    url: "/vedioType/vedioTypeList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="vedioType"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function del(vedioTypeid, status) {
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/vedioType/deleteVedioType/' + vedioTypeid,
            dataType: 'json',
...
function edit(name) {
    $.post("/vedioType/findVedioType/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/vedioType/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/vedioType/vedioTypeList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/vedioType/vedioTypeList'});
}
